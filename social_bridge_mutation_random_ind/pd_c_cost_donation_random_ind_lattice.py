...
def initialize_population():
    ...
    for i in range(total_num):
        popu.append(Agent(i, network[i], np.random.choice([0, 1, 2, 3])))  #  3 for random individual, who's strategy is random.
...
def evolution_one_step(popu, total_num, edges, b, cost):
    ...
    for i in range(total_num):
        ...
        for j in play_agent_l:
            r_i, r_j = pd_game_c_cost_donation(popu[i].get_strategy(), popu[j].get_strategy(), b)
            popu[i].add_payoffs(r_i)
            popu[j].add_payoffs(r_j)
    for i in range(total_num):
...
if __name__ == "__main__":
    simulation_name = "pd_c_cost_donation_local_learn_bridge_lattice"
