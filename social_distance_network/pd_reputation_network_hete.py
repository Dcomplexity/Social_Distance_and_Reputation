...
if __name__ == "__main__":
    log_file_name = "./logs/log_pd_network_hete.txt"
    logger = create_logger(name="pd_network_hete", file_name=log_file_name)

    abs_path = os.path.abspath(os.path.join(os.getcwd(), './'))
    dir_name = abs_path + '/results/'
    if not os.path.isdir(dir_name):
        os.makedirs(dir_name)
    result_file_name = dir_name + "results_pd_network_hete.txt"
    ...
    for _ in range(init_num):
        logger.info(_)
        popu_r, network_r, total_number_r, edges_r = run(b_r, "network_hete", mul_dimen_r, degree_r, group_size_r,
