...
if __name__ == "__main__":
    log_file_name = "./logs/log_pgg_avg_network_hete.txt"
    logger = create_logger(name="pgg_avg_network_hete", file_name=log_file_name)

    abs_path = os.path.abspath(os.path.join(os.getcwd(), './'))
    dir_name = abs_path + '/results/'
    if not os.path.isdir(dir_name):
        os.makedirs(dir_name)
    result_file_name = dir_name + "results_pgg_avg_network_hete.txt"
    f = open(result_file_name, 'w')

    structure_r = "network_hete"
    r_r = 6.0
    init_num = 10
    group_size_r = 50
    group_base_r = 2
