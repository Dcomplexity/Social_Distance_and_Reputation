...
def initialize_population():
    network, total_num, edges = generate_er_random_connected(degree=6, nodes_num=1600)
    popu = []
...
def evolution_one_step(popu, total_num, edges, b, cost):
    ...
    for i in range(total_num):
        ...
        if w2 < w1:
            ...
            w_value = 2.0 * (ind_payoffs - opponent_payoffs)
            if abs(w_value) > 100:
                w_value = (w_value / abs(w_value)) * 100
            t1 = 1 / (1 + math.e ** w_value)
            t2 = random.random()
            if t2 < t1:
...
if __name__ == "__main__":
    simulation_name = "pd_c_cost_b_local_learn_bridge_er_random_6"
