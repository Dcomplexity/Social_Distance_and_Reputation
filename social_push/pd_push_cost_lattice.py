...
def evolution_one_step(popu, total_num, edges, b):
    ...
    for i in range(total_num):
        ...
        while True:
            ...
            # if random.random() < w and (popu[i].get_strategy() == 2 or popu[i].get_strategy() == 3) and \
                    #(popu[j].get_strategy() == 1 and popu[j].get_strategy() == 3):
            if random.random() < w and (popu[i].get_strategy() == 2 or popu[i].get_strategy() == 3):
                # popu[j].add_payoffs(-0.5)
                k = j
            else:
                break
