...
def evolution_one_step(popu, total_num, edges, b, cost):
    ...
    poten_agent = [[] for _ in range(total_num)]
    for i in range(total_num):
        ...
        for j in play_agent_l:
            ...
            if popu[i].get_strategy() == 2:
                if j_action == 1 or j_action == 2:
                    poten_agent[i].append(j)
    for i in range(total_num):
        if popu[i].get_strategy() == 2:
            if len(poten_agent[i]) > 1:
                poten_combination = itertools.combinations(poten_agent[i], 2)
...
if __name__ == "__main__":
    simulation_name = "pd_c_cost_donation_cheat_ind_lattice"
